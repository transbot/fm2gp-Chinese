...
import { bubbleSortVisualization, BubbleSortInput, BubbleSortState } from '../../lib/algorithms/bubble_sort';
...
export function BubbleSort() {
  ...
  const [steps, setSteps] = useState<Step<BubbleSortState>[]>([]);
  const [currentStep, setCurrentStep] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1);
  const [validationErrorKey, setValidationErrorKey] = useState<string | null>(null);

  const parseInput = useCallback((): BubbleSortInput => {
  ...
  const setStepsFromInput = useCallback((input: BubbleSortInput) => {
    const validation = bubbleSortVisualization.validateInput(input);
    if (!validation.valid) {
      setSteps([]);
      setCurrentStep(0);
      setIsPlaying(false);
    ...
    setSteps(bubbleSortVisualization.generateSteps(input));
    setCurrentStep(0);
    setIsPlaying(false);
  }, []);
  ...
  const currentState = steps[currentStep]?.state ?? bubbleSortVisualization.getInitialState();
  ...
  const getCellColor = (index: number) => {
    ...
    if (index >= currentState.sortedFrom) return 'bg-teal-100 border-teal-300';
    if (
      (index === currentState.leftIndex || index === currentState.rightIndex) &&
      operation === 'swap'
    ) {
    ...
    if (
      ...
      return 'bg-yellow-300 ring-2 ring-yellow-500';
    }
  ...
  return (
    <AlgorithmLayout
      ...
      descriptionKey="bubbleSortDescription"
      devNoteKey="devNoteBubbleSort"
      algorithm={bubbleSortVisualization}
    >
      ...
      <div className="bg-gray-50 rounded-xl p-6 border border-gray-200 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-center text-sm text-gray-600">
          <div>{t.pass}: <span className="font-mono font-semibold">{currentState.pass}</span></div>
      ...
      <ExplanationPanel
        stepDescription={
          currentStepData
            ? bubbleSortVisualization.describeStep(currentStepData, lang)
            : t.bubbleSortReady
        }
        invariant={bubbleSortVisualization.getInvariant?.(lang)}
        complexity={bubbleSortVisualization.getComplexity()}
