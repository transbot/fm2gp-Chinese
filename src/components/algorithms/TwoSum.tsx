...
import { twoSumVisualization, TwoSumInput, TwoSumState } from '../../lib/algorithms/two_sum';
...
export function TwoSum() {
  ...
  const [steps, setSteps] = useState<Step<TwoSumState>[]>([]);
  const [currentStep, setCurrentStep] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1);
  const [validationErrorKey, setValidationErrorKey] = useState<string | null>(null);

  const parseInput = useCallback((): TwoSumInput => {
    ...
    return {
      array,
      target: parseInt(targetInput) || 0,
    };
  }, [arrayInput, targetInput]);

  const setStepsFromInput = useCallback((input: TwoSumInput) => {
    const validation = twoSumVisualization.validateInput(input);
    if (!validation.valid) {
      setSteps([]);
      setCurrentStep(0);
      setIsPlaying(false);
    ...
    setSteps(twoSumVisualization.generateSteps(input));
    setCurrentStep(0);
    setIsPlaying(false);
  }, []);
  ...
  const currentState = steps[currentStep]?.state ?? twoSumVisualization.getInitialState();
  const currentStepData = steps[currentStep] ?? null;
  const totalSteps = steps.length;
  const seenEntries = Object.entries(currentState.seen).sort(([, a], [, b]) => a - b);
  ...
  const generateSampleArray = useCallback(() => {
    const array = [4, 1, 9, 6, 3, 8];
    const target = 12;
    setArrayInput(array.join(', '));
    setTargetInput(String(target));
    setStepsFromInput({ array, target });
  }, [setStepsFromInput]);
  ...
  const getCellColor = (index: number) => {
    if (currentState.result?.includes(index)) return 'bg-green-300 ring-2 ring-green-500';
    if (index === currentState.currentIndex) return 'bg-yellow-300 ring-2 ring-yellow-500';
  ...
  return (
    <AlgorithmLayout
      ...
      descriptionKey="twoSumDescription"
      devNoteKey="devNoteTwoSum"
      algorithm={twoSumVisualization}
    >
      ...
      <div className="bg-gray-50 rounded-xl p-6 border border-gray-200 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-2 text-sm text-gray-600">
            ...
            <div>{t.complement}: <span className="font-mono font-semibold">{currentState.complement ?? '-'}</span></div>
            <div>{t.result}: <span className="font-mono font-semibold text-blue-700">{currentState.result ? `[${currentState.result.join(', ')}]` : '-'}</span></div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-sm font-semibold text-gray-700 mb-3">{t.seenValues}</div>
            <div className="space-y-2">
              {seenEntries.length === 0 ? (
                <div className="text-sm text-gray-500">{t.empty}</div>
              ) : (
                seenEntries.map(([value, index]) => (
                  <div key={`${value}-${index}`} className="flex items-center justify-between text-sm">
                    <span className="font-mono text-gray-700">{value}</span>
                    <span className="font-mono font-bold text-blue-700">[{index}]</span>
                  </div>
                ))
              )}
            </div>
      ...
      <ExplanationPanel
        stepDescription={
          currentStepData
            ? twoSumVisualization.describeStep(currentStepData, lang)
            : t.twoSumReady
        }
        invariant={twoSumVisualization.getInvariant?.(lang)}
        complexity={twoSumVisualization.getComplexity()}
